...
from test_depth import from_pxl_to_camera
...
# 这里还未补充Mocap的代码
detector_name = "Mediapipe" # "Mediapipe" or "MoCap"
...
def start_retargeting(queue: multiprocessing.Queue, robot_dir: str, config_path: str,
                      robot_name: RobotName, retargeting_type: RetargetingType, 
                      hand_type: HandType, headless: bool = False, multi_cam: bool = False):
    global current_key # 全局变量，用于存储当前按下的键
    ...
    # cam.set_local_pose(sapien.Pose([1.5, -0.2, 0.5], [0, 0, 0, -1])) # 从+x轴观测
    # cam.set_local_pose(sapien.Pose([0.4, 0.2, 1.6], [0.924, 0, 0.383, 0])) # 从-x轴观测
    # cam.set_local_pose(sapien.Pose([0, -1, 0.3], [0.707, 0, 0, 0.707])) # 从-y轴观测
    # cam.set_local_pose(sapien.Pose([0.35, 0.15, 1.4], [0.961, -0.025, 0.258, -0.095])) # 从-x轴观测，略微调整位置和角度以获得更好视角
    # cam.set_local_pose(sapien.Pose([0.25, 0.25, 1.3], [0.947, -0.05, 0.254, -0.188])) # 从-x轴观测，略微调整位置和角度以获得更好视角
    # cam.set_local_pose(sapien.Pose([0.35, 0.35, 1.2], [1, 0, 0, 0])) 
    ...
    loader.collision_material = scene.create_physical_material(
        static_friction=30.0,    # 静摩擦（越大越不滑）
        dynamic_friction=20.0,   # 动摩擦
        restitution=0.0         # 不弹
    )
    ...
    # init_qpos[:7]=[0,0.21,0,2.04,0,0.06,0] # 机械臂初始位置，单位为弧度
    init_qpos[:7]=[0,0.21,0,0.9,0,-0.75,0]
    init_qpos_copy = init_qpos.copy()
    ...
    is_init = False
    last_rot = R.from_euler('zyx', [-90, 0, 180], degrees=True)
    take_photo = False
    ...
    try:
        while True:
            ...
            if current_key == "q":
                ...
                depth += 0.01
                logger.info(f"🔼 Depth increased: {depth:.2f}m")
                current_key = None
            elif current_key == "s":
                depth = max(0.01, depth - 0.01)
                logger.info(f"🔽 Depth decreased: {depth:.2f}m")           
                current_key = None
                ...
                logger.info(f"📌 Current Pose: Position {controller.pose.translation}")
                logger.info(f"📌 Current Pose: Rotation {R.from_matrix(controller.pose.rotation).as_quat()}")
                current_key = None
                ...
                logger.info("  E: Cycle Pre-set Wrist Poses (if enabled)")
                logger.info("  F: Toggle Pre-set Wrist Poses")
                logger.info("  R: Reset Controller and Scene")
                logger.info("  T: Reset and Switch YCB Object")
                logger.info("  W/S: Increase/Decrease Depth")
                logger.info("  Arrow Keys: Move Origin Position (Hold for Continuous Movement)")
                logger.info("  Space/C: Move Origin Up/Down")
                logger.info("  P: Print Current Pose")
            ...
            if joint_pos is None:
                ...
                if detector_name == "MoCap":
                    # Mocap系统直接提供像素坐标
                    wrist_pos_pxl = keypoint_2d[0]
                else:
                    # MediaPipe提供的是NormalizedLandmarkList，需要转换为像素坐标
                    wrist_pos_pxl = landmarks_to_pixel_array(keypoint_2d, rgb.shape)[0]  # 获取腕部关键点的像素坐标
                
                wrist_pos = from_pxl_to_camera(wrist_pos_pxl, depth)

                if not is_init:
                    init_position = wrist_pos @ CAMERA2WORLD.T
                    is_init = True
                    # wrist_pos_m[0] -= init_depth  # 根据初始深度调整x坐标，使其更接近机械臂末端初始位置
                    # wrist_pos_m[1] -= init_depth

                # 目标腕部姿态 [x, y, z, qw, qx, qy, qz]
                wrist_pos_m = (wrist_pos @ CAMERA2WORLD.T - init_position)*2 + origin_pos
            ...
            for _ in range(2):
                ...
                if multi_cam or take_photo or i % 100 == 0:
                    # 渲染所有相机图像
                    images = {}
            ...
            if joint_pos is not None:
                ...
                h, w = rgb.shape[:2]
                current_keypoint_2d = landmarks_to_pixel_array(keypoint_2d, (h, w)) # 手部关键点像素坐标
...
def main(
    ...
    args=(queue, str(robot_dir), str(config_path), robot_name, retargeting_type, hand_type, headless, multi_cam)
)
