...
def compute_camera_pose(cam_pos, lookat = [0, 0, 0]) -> sapien.Pose:
...
def main():
    ...
    urdf_path = r"D:\study\VScodes\Retargeting\assets\robots\assembly\xarm7_qbr\qbr.urdf"
    ...
    depth_pil.save("data/camera/depth.png")
    ...
    label0_pil.save("data/camera/label0.png")
    label1_pil = Image.fromarray(color_palette[label1_image])
    label1_pil.save("data/camera/label1.png")
    ...
    while not viewer.closed:
        if viewer.window.key_down("p"):  # Press 'p' to take the screenshot
            ...
            rgba_img = (rgba * 255).clip(0, 255).astype("uint8")
            rgba_pil = Image.fromarray(rgba_img)
            rgba_pil.save("data/camera/screenshot.png")
        scene.step()
        scene.update_render()
        viewer.render()
