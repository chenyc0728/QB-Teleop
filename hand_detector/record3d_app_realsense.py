...
class RealsenseApp:
    ...
    def connect_to_device(self):
        if not os.path.exists(self.rgb_video_file):
            ...
            # ⭐ 1. 获取 color sensor 对象
            # 传感器列表中，索引 1 通常是 color sensor
            color_sensor = dev.get_device().query_sensors()[1]
            
            # ⭐ 2. (可选) 打印当前曝光值，以便调试
            current_exp = color_sensor.get_option(pyrs.option.exposure)
            print(f"Current exposure: {current_exp}")

            # ⭐ 3. 关闭自动曝光
            # 可选。关闭自动曝光优先级，使曝光值更稳定[reference:4]
            # color_sensor.set_option(pyrs.option.auto_exposure_priority, False)
            color_sensor.set_option(pyrs.option.enable_auto_exposure, False)
            
            # ⭐ 4. 手动设置曝光和增益 (你也可以从RealSense Viewer测试确定值)
            # 以下值仅为示例，请根据你的实际效果调整
            color_sensor.set_option(pyrs.option.exposure, 200.0)  # 增加这个值使图像更亮
            color_sensor.set_option(pyrs.option.gain, 64.0)      # 配合增益使用
            # 可选，设置伽马，也影响亮度表现[reference:5]
            # color_sensor.set_option(pyrs.option.gamma, 300.0)
    ...
    def fetch_rgb_and_depth(self):
        if not self.rgb_video_file:
            ...
            while self.rgb_stream.isOpened():
                ...
                if not success_rgb:
                    print("RGB video ended.")
                    return None, None
                if self.read_count >= len(self.depth_stream):
                    print("Depth data exhausted.")
                    return None, None
...
if __name__ == '__main__':
    ...
    # filename = f"data/video/rgb_{1:0>4d}.mp4"
    filename = f"D:/study/VScodes/Retargeting/data/video/rgb_{time.strftime('%Y%m%d_%H%M%S')}.mp4"
